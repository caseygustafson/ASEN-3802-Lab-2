...
%% Part 2 Task 3
% This section computes and plots the transient analytical solution (Model IB)
...
for i = 1:length(files)  % Loop through all 5 datasets again
    ...
    % Model parameters
    H_exp = H_exp_list(i);          % Select experimental steady-state slope for current dataset
    ...
    for j = 1:8                   % Loop over the 8 thermocouple locations
        ...
        for n = 1:N_modes         % Loop over the retained Fourier modes
            ...
            b_n = (8 * H_exp * L * (-1)^n) / (((2*n - 1)^2) * pi^2);
    ...
    title(sprintf('Model IB Temperature vs Time — %s', strrep(files(i), '_', ' ')));
    % Create plot title
