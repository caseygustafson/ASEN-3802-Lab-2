...
% M_exp values from Part 1 Task 2 of Aluminum 25V, Aluminum 30V, Brass 25V,
% Brass 30V, and Steel 22V in that order
M_exp_vals = [0.468691, 0.093738, 4.311961, 4.874391, 18.466442];

% Initial-condition y-intercept from Part 1 Task 2
T0_init_vals = [16.600298, 16.530060, 16.307738, 16.363095, 16.871726];
...
%% Part 2 Task 4
% This section computes and plots the transient analytical solution (Model II)
...
for i = 1:length(files)  % Loop through all 5 datasets again
    ...
    % Model parameters
    H_exp = H_exp_list(i);      % steady-state slope from Part 1 Task 1
    M_exp = M_exp_vals(i);      % Initial condition slope from Part 1 Task 2
    T0    = T0_init_vals(i);    % initial-condition y-intercept from Part 1 Task 2
    ...
    for j = 1:8                   % Loop over the 8 thermocouple locations
        ...
        for n = 1:N_modes         % Loop over the retained Fourier modes
            ...
            % Accounting for linear initial condition
            b_n = (8 * (M_exp - H_exp) * L * (-1)^(n+1)) / (((2*n-1)^2) * pi^2);
    ...
    title(sprintf('Model II Temperature vs Time — %s', strrep(files(i), '_', ' ')));
    % Create plot title
    ...
    text(0.02, 0.98, sprintf('H_{exp} = %.3f ^\\circC/m\nM_{exp} = %.3f ^\\circC/m\nT_0 = %.3f ^\\circC\nN = %d', ...
        H_exp, M_exp, T0, N_modes), ...
